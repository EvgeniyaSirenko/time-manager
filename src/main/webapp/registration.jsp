		...
		<form action="controller" method="post">
			...
			<input	type="hidden" name="roleId" value="1" /> 			
				<label for="firstName"><fmt:message	key="participant_account_jsp.label.first_name" /></label> 
					<input	type="text" pattern="[A-Za-zа-яА-Я]{3,15}" oninvalid="setCustomValidity('Please use only Latin letters, 3-15 symbols')" name="firstName" required /><br>
				 <label	for="lastName"><fmt:message	key="participant_account_jsp.label.last_name" /></label> 
					<input	type="text" pattern="[A-Za-zа-яА-Я]{3,15}" oninvalid="setCustomValidity('Please use only Latin letters, 3-15 symbols')" name="lastName" required /><br>
				<label for="login"><fmt:message	key="login_jsp.label.login" /></label> 
					<input type="text" pattern="[A-Za-zа-яА-Я]{3,15}" oninvalid="setCustomValidity('Please use only Latin letters, 3-15 symbols')" name="login" required /><br>
				<label for="password"><fmt:message key="login_jsp.label.password" /></label> 
					<input type="password" pattern="[A-Za-z0-9]{3,15}" oninvalid="setCustomValidity('Please use numbers and Latin letters, 5-15 symbols')" name="password" required /><br> 
